...
function ChatPanel() {
  ...
  const normalizeExtractedData = (payload) => {
    if (!payload) return null;
    if (payload.section) {
      return { [payload.section]: payload.data };
    }
    return payload;
  };

  const applyExtractedData = (payload) => {
    const normalized = normalizeExtractedData(payload);
    if (!normalized) return false;

    Object.entries(normalized).forEach(([section, data]) => {
  ...
  const handleSend = async (text) => {
    ...
    try {
      ...
      const extractedData = normalizeExtractedData(response.extractedData);
      const wasApplied = applyExtractedData(extractedData);
