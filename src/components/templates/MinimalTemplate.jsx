        ...
        <div style={styles.sectionGap}>
          ...
          {education.map((edu, i) => (
            <div key={i} style={{ ...styles.entryRow, marginBottom: '6px' }}>
              <div>
                <span style={{ fontSize: '14px', fontWeight: 600, color: palette.text }}>{edu.degree}</span>
                {edu.field && <span style={{ fontSize: '13px', color: palette.secondary }}> in {edu.field}</span>}
                {edu.institution && <span style={{ fontSize: '13px', color: palette.muted }}> — {edu.institution}</span>}
                {edu.gpa && <span style={{ fontSize: '12px', color: palette.muted }}> (GPA: {edu.gpa})</span>}
              </div>
              <span style={styles.dates}>
            ...
            {projects.map((proj, i) => (
              <div key={i} style={{ marginBottom: '14px' }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
                  <span style={{ fontSize: '14px', fontWeight: 600, color: palette.text }}>{proj.name}</span>
                ...
                {proj.technologies?.length > 0 && (
                  <p style={{ fontSize: '12px', color: palette.muted, margin: '2px 0' }}>{proj.technologies.join(', ')}</p>
                )}
            ...
            {certifications.map((cert, i) => (
              <div key={i} style={{ ...styles.entryRow, marginBottom: '4px' }}>
                <div style={{ fontSize: '13px' }}>
                  <span style={{ fontWeight: 600, color: palette.text }}>{cert.name}</span>
                  {cert.issuer && <span style={{ color: palette.muted }}> — {cert.issuer}</span>}
